...
class VoteInteractionHandler {
  ...
  constructor(
    private chConfig: ChSettingsData,
    private interaction: ButtonInteraction<CacheType>,
  ...
  private isGraterThanVotThreshold = (votes: number): boolean =>
    votes >= (this.chConfig.voting_threshold ?? 0);

  private isGraterThanApprThreshold = (approvals: number): boolean =>
    approvals >= (this.chConfig.approval_threshold ?? 0) ||
    !isApprovable(this.chConfig);

  private isEnoughSubmissions = async (): Promise<boolean> => {
    const threshold = this.chConfig.submission_threshold ?? 0;
    if (threshold > 0) {
      const totalRes = await this.managers.documents.getNumOfDocsPerUserId({
  ...
  private assignRole = async (
    voteNum: number,
    apprNum: number,
    innMessage: VotingMessage
  ): Promise<void> => {
    ...
    if (
      ...
      if (member) {
        const repliedToMsg = message.reference?.messageId
          ? await fetchMessageById(message, message.reference.messageId)
          : null;
        ...
        if (link && type && this.interaction.message.type === 'REPLY') {
          ...
          await this.managers.documents.patchByMsgId(
  ...
  process = async (): Promise<
    string | MessagePayload | InteractionUpdateOptions | null | false
  > => {
    ...
    try {
      ...
      if (this.type === 'dismiss' && isAllowedToApprove) {
        if (msg.type === 'REPLY' && msg.deletable) {
          await msg.delete();
          await this.managers.documents.deleteByMessageId({
            message_id: msg.id,
          });
          return false;
        }
      ...
      if (
        ((this.type === 'like' || this.type === 'dislike') &&
          !isAllowedToVote) ||
      ...
      if (message.embeds[0]) {
        ...
        const vts = await this.managers.votes.getVoteCountsByMessageId({
          message_id: msg.id,
        });
        const apprs = await this.managers.votes.getVoteCountsByMessageId({
          message_id: msg.id,
          is_approval: true,
        });
        ...
        if (innMessage) {
          const components = [
            ...
            ...(isAppr
              ? [
                  genApproveButton(
                    this.chConfig.approval_threshold ?? 0,
                    apprs?.in_favor_count ?? 0
                  ),
                ]
              : []),
          ];

          // console.log(this.chConfig.approval_threshold);
          // console.log(apprs?.in_favor_count);

          if (
            isAppr &&
            this.chConfig.approval_threshold &&
            this.chConfig.approval_threshold > 0 &&
            ((apprs?.in_favor_count && apprs?.in_favor_count === 0) ||
              !apprs?.in_favor_count)
          ) {
            console.log('generating the dismiss button');
            components.push(genDismissButton());
          }
          ...
          if (this.type === 'like' || this.type === 'approve') {
            await this.assignRole(
              (vts?.in_favor_count ?? 0) - (vts?.against_count ?? 0),
              apprs?.in_favor_count ?? 0,
              innMessage
            );
          }
